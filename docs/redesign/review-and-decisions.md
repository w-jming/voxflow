...
| D-22 | 默认 ASR 后端 | 所有者 2026-06-11 裁决:**Qwen3-ASR-1.7B + vLLM 为默认配置;同时提供火山引擎 API 选项,用户可自由切换** | 已实现:`config.asr.backend ∈ {qwen3_vllm(默认), volcano_api, zipformer_local, mock}`;Qwen3 经 Python sidecar(`sidecar/qwen3_asr_sidecar.py`,官方 `qwen-asr[vllm]` 流式 API,JSONL stdio)接 `voxflow-asr-qwen3`;火山经 wss 二进制协议接 `voxflow-asr-volcano`(密钥仅存用户配置);控制中心「输入」页切换。**⚠️ 火山 API 路径未经真实服务验证**(2026-06-12:无可用密钥,不具备测试条件;协议按公开文档与社区参考实现编写,仅有离线协议帧/映射单测。首次真实接入时按 [stage-7 前置项] 验证并更新本标注)。注:这调整了 PRD"轻量即用"默认层(zipformer 降为兜底选项),运行时引入 Python/vLLM 依赖 | **所有者裁决 ✦** |
