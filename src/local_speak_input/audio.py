...
def _write_wav(frames: list[bytes], sample_rate: int) -> Path:
    tmp = tempfile.NamedTemporaryFile(prefix="local-speak-", suffix=".wav", delete=False)
    path = Path(tmp.name)
    tmp.close()
...
class PipeWireRecorder:
    ...
    def record_once(self) -> Path:
        tmp = tempfile.NamedTemporaryFile(prefix="local-speak-pw-", suffix=".wav", delete=False)
        path = Path(tmp.name)
        tmp.close()
