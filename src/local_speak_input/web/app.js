...
async function loadConfig() {
  ...
  const hotkey = config.daemon?.hotkey || "ctrl+alt+space";
  el.configLine.textContent = `${config.asr.backend} · ${config.asr.model} · ${config.asr.language} · ${formatHotkey(hotkey)}`;
...
async function saveHotkey() {
  ...
  const response = await fetch("/api/settings/hotkey", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ hotkey, restart: true }),
  });
  const payload = await response.json();
  if (!response.ok) {
...
el.saveHotkeyBtn.addEventListener("click", saveHotkey);
